...
public class SettingsChangedEvent : CompositePresentationEvent<NoArgs>
{
    ...
    {
        return FrameworkApplication.EventAggregator.GetEvent<SettingsChangedEvent>()
            .Register(action, keepSubscriberAlive);
    }
        
    public static void Unsubscribe(Action<NoArgs> action) => FrameworkApplication.EventAggregator.GetEvent<SettingsChangedEvent>().Unregister(action);
        
    public static void Unsubscribe(SubscriptionToken token) => FrameworkApplication.EventAggregator.GetEvent<SettingsChangedEvent>().Unregister(token);

    internal static void Publish(NoArgs eventArgs) => FrameworkApplication.EventAggregator.GetEvent<SettingsChangedEvent>().Broadcast(eventArgs);
}
