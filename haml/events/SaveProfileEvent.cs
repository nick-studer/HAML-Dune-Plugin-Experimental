...
namespace HamlProAppModule.haml.events
{
    ...
    public class SaveProfileEvent : CompositePresentationEvent<Profile>
    {
        ...
        {
            return FrameworkApplication.EventAggregator.GetEvent<SaveProfileEvent>()
                .Register(action, keepSubscriberAlive);
        }
        
        public static void Unsubscribe(Action<Profile> action) => FrameworkApplication.EventAggregator.GetEvent<SaveProfileEvent>().Unregister(action);
        
        public static void Unsubscribe(SubscriptionToken token) => FrameworkApplication.EventAggregator.GetEvent<SaveProfileEvent>().Unregister(token);

        internal static void Publish(Profile eventArgs) => FrameworkApplication.EventAggregator.GetEvent<SaveProfileEvent>().Broadcast(eventArgs);
    }
}
